...
\fi
\usepackage[]{natbib}
\bibliographystyle{juliacon.bst}
...
unstable \citep{goodfellow2014explaining}, encode existing biases
\citep{buolamwini2018gender} and learn representations that are
surprising or even counter-intuitive from a human perspective
\citep{buolamwini2018gender}. Nonetheless, they often form the basis for
...
agent---i.e.~the black-box system \citep{borch2022machine}. The group of
...
\citep{arrieta2020explainable}; Fan et al.~(2020) focus on
explainability in the context of deep learning
\citep{fan2020interpretability}; and finally, Karimi et al.~(2020)
\citep{karimi2020survey} and Verma et al.~(2020)
\citet{verma2020counterfactual} offer detailed reviews of the literature
on Counterfactual Explanations and Algorithmic Recourse (see also
\citet{molnar2020interpretable} and \citet{varshney2022trustworthy}).
Miller (2019) explicitly discusses the concept of explainability from
the perspective of a social scientist \citep{miller2019explanation}.
...
distinction made in \citet{rudin2019stop} useful: Interpretable AI
...
\citet{rudin2019stop}. While we agree that initial efforts should always
...
\citep{molnar2020interpretable}. Tools for global explainability include
...
\citep{kaur2020interpreting}. Another recent work has shown that both
...
strongly biased black-box models \citep{slack2020fooling}. In related
...
protect against this in practice \citep{slack2021counterfactual}. In the
...
\citep{molnar2020interpretable}. Typically the underlying methodology is
...
and regression \citep{molnar2020interpretable}.

The counterfactual search objective originally proposed by
\citet{wachter2017counterfactual} is as follows
...
were first proposed in 2017 (see \citet{verma2020counterfactual} and
\citet{karimi2020survey} for surveys). The various approaches largely
...
\citep{wachter2017counterfactual}; \textbf{actionability} --- the
proposed recourse should be actionable
\citep{ustun2019actionable, poyiadzi2020face}; \textbf{plausibility} ---
the counterfactual explanation should be plausible to a human
\citep{joshi2019realistic, schut2021generating}; \textbf{sparsity} ---
the counterfactual explanation should involve as few individual feature
changes as possible \citep{schut2021generating}; \textbf{robustness} ---
the counterfactual explanation should be robust to domain and model
shifts \citep{upadhyay2021robust}; \textbf{diversity} --- ideally
multiple diverse counterfactuals should be provided
\citep{mothilal2020explaining}; and \textbf{causality} ---
counterfactuals should respect the structural causal model underlying
the data generating process
\citep{karimi2020algorithmic, karimi2021algorithmic}.
...
\citep{pawelczyk2021carla}.\footnote{While we were writing this paper,
  the \texttt{R} package \texttt{counterfactuals} was released
  \citep{dandl2023counterfactuals}. The developers seem to also envision
...
proposed in \citet{schut2021generating} has been released. As another
...
in \href{https://fluxml.ai/}{Flux} \citep{innes2018flux} as well as a
small subset of models made available through
\href{https://alan-turing-institute.github.io/MLJ.jl/dev/}{MLJ}
\citep{blaom2020mlj}. While in general it is assumed that users resort
to this package to explain their pre-trained models, we provide a simple
...
\item
  Deep Ensemble \citet{lakshminarayanan2016simple}
\item
...
\citep{joshi2019realistic, karimi2021algorithmic}. While this notion is
...
\citep{altmeyer2023endogenous}; \emph{CLUE} searches plausible
counterfactuals in the latent embedding of a generative model by
explicitly minimising predictive entropy \citep{antoran2020getting};
\emph{DiCE} is designed to generate multiple, maximally diverse
counterfactuals \citep{mothilal2020explaining}; \emph{FeatureTweak}
...
features in the least costly way \citep{tolomei2017interpretable};
...
target class \citep{altmeyer2023endogenous}; \emph{Greedy} aims to
generate plausible counterfactuals by implicitly minimising predictive
uncertainty of Bayesian classifiers \citep{schut2021generating};
...
direction\citep{laugel2017inversea}; \emph{PROBE} generates
probabilistically robust counterfactuals
\citep{pawelczyk2022probabilistically}; \emph{REVISE} addresses the need
for plausibility by searching counterfactuals in the latent embedding of
a Variational Autoencoder (VAE) \citep{joshi2019realistic};
\emph{Wachter} is the baseline approach that only penalises the distance
to the original sample \citep{wachter2017counterfactual}.
...
\bottomrule\noalign{}
\endlastfoot
ClaPROAR \citep{altmeyer2023endogenous} & gradient based & feature &
yes \\
CLUE \citep{antoran2020getting} & gradient based & latent & yes \\
DiCE \citep{mothilal2020explaining} & gradient based & feature & yes \\
FeatureTweak \citep{tolomei2017interpretable} & tree based & feature &
no \\
Gravitational \citep{altmeyer2023endogenous} & gradient based & feature
& yes \\
Greedy \citep{schut2021generating} & gradient based & feature & yes \\
GrowingSpheres \citep{laugel2017inversea} & agnostic & feature & no \\
PROBE \citep{pawelczyk2022probabilistically} & gradient based & feature
& no \\
REVISE \citep{joshi2019realistic} & gradient based & latent & yes \\
Wachter \citep{wachter2017counterfactual} & gradient based & feature &
yes \\
\end{longtable}
...
\tightlist
\item
  Adult Census \citep{becker1996adult}
\item
  California Housing \citep{pace1997sparse}
\item
  CIFAR10 \citep{krizhevsky2009learning}
\item
  German Credit \citep{hoffman1994german}
\item
  Give Me Some Credit \citep{kaggle2011give}
\item
  MNIST \citep{lecun1998mnist} and Fashion MNIST \citep{xiao2017fashion}
\item
  UCI defaultCredit \citep{yeh2009comparisons}
\end{itemize}
...
by \citet{mothilal2020explaining} that induces diverse sets of
...
distance metric defined in \citet{wachter2017counterfactual}, for
...
\citep{innes2018flux} deep learning models was enabled.\footnote{Flux
...
\citep{delaney2021uncertainty}. One idea could be, to use dropout to
...
datasets that ship with the package \citep{kaggle2011give}. It can be
...
\citep{altmeyer2023endogenous} we will generate counterfactuals for ten
...
28x28 pixel grey-scale images \citep{lecun1998mnist}. Each image is
...
Learning framework for Julia \citep{blaom2020mlj}. This may also involve
...
including MINT \citep{karimi2021algorithmic}, ROAR
\citep{upadhyay2021robust} and FACE \citep{poyiadzi2020face}.
...
\emph{German Credit} \citep{karimi2020survey}. There is scope for adding
